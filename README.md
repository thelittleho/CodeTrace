...
## 💻 사용 방법
1. **로그인**: GitHub 계정을 연동하여 CodeTrace에 로그인합니다.
2. **문제 가져오기**: 새 노트를 생성한 뒤, 풀이하고자 하는 프로그래머스 문제 URL을 입력하여 문제 설명과 제목을 자동으로 불러옵니다.
3. **코드 첨부 및 노트 작성**: GitHub 탐색기 기능을 이용해 본인 저장소에 푸시된 정답 소스 코드를 클릭 한 번으로 노트에 첨부합니다.
4. **저장 및 동기화**: 마크다운 에디터를 활용해 핵심 풀이 로직을 기록합니다. 작성된 노트는 클라우드에 실시간으로 안전하게 동기화됩니다.
